...
Socrates is installed in this project. Pick the protocol that matches how you're working and
read it at the start of the session before doing anything else:

- **Autonomous loop** (`ralph.sh` / `ralph-once.sh` driving): read `RALPH.md`.
...
All other discipline (role triage, phase sequence, handoff/ADR rules, `.msgs/` inbox,
interactive deltas) lives in those protocol files. Do not duplicate it here.
